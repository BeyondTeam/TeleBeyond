...
local apibot = 231281320
   if msg.action.user.id == tonumber(apibot) then
  return nil
  end
local bot = 209697172
   if msg.action.user.id == tonumber(bot) then
  return nil
  end
      if not msg.action.user.username then
          nama = string.gsub(msg.action.user.first_name, "_", " ")
      else 
          nama = msg.action.user.first_name
      end
      chat_new_user(msg)
   ...
   elseif matches[1] == "chat_add_user_link" then
      ...
      if not msg.from.username then
          nama = string.gsub(msg.from.first_name, "_", " ")
      else
